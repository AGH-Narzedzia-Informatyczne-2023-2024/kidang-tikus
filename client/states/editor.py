...
class EditorState(State):
    ...
    def update(self):
        ...
        # Controls
        self.exitText, self.exitTextRect = self.fonts.draw_text("Exit", (255, 255, 255), "Arial30", self.editorSurface,
                                                                (25, self.game.GAME_SIZE[1] - 25),
                                                                positionProp='bottomleft')
        self.deleteText, self.deleteTextRect = self.fonts.draw_text("Delete", (255, 255, 255), "Arial30",
                                                                    self.editorSurface,
                                                                    (200, self.game.GAME_SIZE[1] - 25),
                                                                    positionProp='bottomleft')
        if self.tileSelectIndex != 0:
            self.leftText, self.leftTextRect = self.fonts.draw_text("<", (255, 255, 255), "Arial30", self.editorSurface,
                                                                    (50, self.controlsOffset + 50),
                                                                    positionProp='midright')
        if self.tileSelectIndex < len(TileList) - SelectTilesShown:
            self.rightText, self.rightTextRect = self.fonts.draw_text(">", (255, 255, 255), "Arial30",
                                                                      self.editorSurface, (60 + SelectTilesShown * 60,
                                                                                           self.controlsOffset + 50),
                                                                      positionProp='midleft')

        self.saveText, self.saveTextRect = self.fonts.draw_text("Save", (255, 255, 255), "Arial30", self.editorSurface,
                                                                (800, self.controlsOffset + 30), positionProp='topleft')
        self.loadText, self.loadTextRect = self.fonts.draw_text("Load", (255, 255, 255), "Arial30", self.editorSurface,
                                                                (870, self.controlsOffset + 30), positionProp='topleft')

        if self.loadsaveMsg is not None:
            self.fonts.draw_text(self.loadsaveMsg, (255, 255, 255), "Arial30", self.editorSurface,
